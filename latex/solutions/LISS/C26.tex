...
\end{align*}
%
Since there is a leading one in each row of the row-reduced coefficient matrix, there is a solution for every vector $\colvector{x\\y\\z\\w}$, which means that there is a solution to the original equation for every matrix $\begin{bmatrix} x & y\\ z & w \end{bmatrix}$.  Thus, the original five matrices span $M_{2,2}$.
%
