...
Suppose $A$ is an $m\times n$ matrix.  Add $m$ new columns to $A$ that together equal an $m\times m$ identity matrix to form an $m\times(n+m)$ matrix $M$.  Use row operations to bring $M$ to reduced row-echelon form and call the result $N$.  $N$ is the \define{extended reduced row-echelon form} of $A$, and we will standardize on names for five submatrices ($B$, $C$, $J$, $K$, $L$) of $N$.\par
%
