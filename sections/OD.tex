...
%% Reference symmetric matrix diagonalization back in eigenvalue section
%% Diagonalize a big, interesting normal matix (skew symmetric?)
%
We have seen in \acronymref{section}{SD} that under the right conditions a square matrix is similar to a diagonal matrix.  We recognize now, via \acronymref{theorem}{SCB}, that a similarity transformation is a change of basis on a matrix representation.  So we can now discuss the choice of a basis used to build a matrix representation, and decide if some bases are better than others for this purpose.  This will be the tone of this section.  We will also see that every matrix has a reasonably useful matrix representation, and we will discover a new class of diagonalizable linear transformations.  First we need some basic facts about triangular matrices.
%
\subsect{TM}{Triangular Matrices}
%
...
The $n\times n$ square matrix $A$ is \define{upper triangular} if $\matrixentry{A}{ij} =0$ whenever $i>j$.
\end{definition}
%
%
\begin{definition}{LTM}{Lower Triangular Matrix}{lower triangular matrix}
The $n\times n$ square matrix $A$ is \define{lower triangular} if $\matrixentry{A}{ij} =0$ whenever $i<j$.
\end{definition}
%
...
\end{align*}
%
Since $\matrixentry{AB}{ij}=0$ whenever $i<j$, by \acronymref{definition}{LTM}, $AB$ is lower triangular.
%
\end{proof}
%
...
%
\begin{proof}
We give the proof for the case when $A$ is lower triangular, and leave the case when $A$ is upper triangular for you.  Consider the process for computing the inverse of a matrix that is outlined in the proof of \acronymref{theorem}{CINM}.  We augment $A$ with the size $n$ identity matrix, $I_n$, and row-reduce the $n\times 2n$ matrix to reduced row-echelon form via the algorithm in \acronymref{theorem}{REMEF}.  The proof involves tracking the peculiarities of this process in the case of a lower triangular matrix.  Let $M=\augmented{A}{I_n}$.\par
%
...
%%%%%%%%%%%  BS hermitian, inner-product stuff
...
%
\subsect{UTMR}{Upper Triangular Matrix Representation}
%
...
%
\begin{proof}
We begin with a proof by induction (\acronymref{technique}{I}) of the first statement in the conclusion of the theorem.  We use induction on the dimension of $V$ to show that if $\ltdefn{T}{V}{V}$ is a linear transformation,  then there is a basis $B$ for $V$ such that the matrix representation of $T$ relative to $B$, $\matrixrep{T}{B}{B}$, is an upper triangular matrix.\par
%
To start suppose that $\dimension{V}=1$.  Choose any nonzero vector $\vect{v}\in V$ and realize that $V=\spn{\set{\vect{v}}}$.  Then we can determine $T$ uniquely by $\lt{T}{\vect{v}}=\beta\vect{v}$ for some $\beta\in\complexes$ (\acronymref{theorem}{LTDB}).  This description of $T$ also gives us a matrix representation relative to the  basis $B=\set{\vect{v}}$ as the $1\times 1$ matrix with lone entry equal to $\beta$.  And this matrix representation is upper triangular (\acronymref{definition}{UTM}).\par
%
For the induction step let $\dimension{V}=m$, and assume the theorem is true for every linear transformation defined on a vector space of dimension less than $m$.  By \acronymref{theorem}{EMHE} (suitably converted to the setting of a linear transformation), $T$ has at least one eigenvalue, and we denote this eigenvalue as $\lambda$.  (We will remark later about how critical this step is.)  We now consider properties of the linear transformation $\ltdefn{T-\lambda I_V}{V}{V}$.\par
%
...
\end{align*}
%
So $T-\lambda I_V$ is not injective, as it has a nontrivial kernel (\acronymref{theorem}{KILT}).  With an application of \acronymref{theorem}{RPNDD} we bound the rank of $T-\lambda I_V$,
%
\begin{align*}
...
\end{align*}
%
Since $W$ is the range of $T-\lambda I_V$, $\lt{\left(T-\lambda I_V\right)}{\vect{w}}\in W$.  And by \acronymref{property}{SC}, $\lambda\vect{w}\in W$.  Finally, applying \acronymref{property}{AC} we see by closure that the sum is in $W$ and so we conclude that $\lt{S}{\vect{w}}\in W$.  This argument convinces us that it is legitimate to define $S$ as we did with $W$ as the codomain.\par
%
...
%
By \acronymref{theorem}{DSFOS} there exists a second subspace of $V$, which we will call $U$, so that $V$ is a direct sum of $W$ and $U$, $V=W\ds U$.  Choose a basis $D=\set{\vectorlist{u}{\ell}}$ for $U$.  So  $m=k+\ell$ by \acronymref{theorem}{DSD}, and $B=C\cup D$ is basis for $V$ by \acronymref{theorem}{DSLI} and \acronymref{theorem}{G}.  $B$ is the basis we desire.  What does a matrix representation of $T$ look like, relative to $B$?\par
%
...
\end{align*}
%
The roots of the polynomial equation $\charpoly{\matrixrep{T}{B}{B}}{x}=0$ are the eigenvalues of the linear transformation (\acronymref{theorem}{EMRCP}).  So each diagonal entry is an eigenvalue, and is repeated on the diagonal exactly $\algmult{T}{\lambda}$ times (\acronymref{definition}{AME}).
%
\end{proof}
%
...
%
In the case of linear transformations defined on $\complex{m}$, we can use the inner product (\acronymref{definition}{IP}) profitably to fine-tune the basis that yields an upper triangular matrix representation.  Recall that the adjoint of matrix $A$ (\acronymref{definition}{A}) is written as $\adjoint{A}$.
%
...
\end{align*}
%
In this form, we recognize that the change-of-basis matrix $\cbm{B}{C}=\matrixrep{I_{\complex{n}}}{B}{C}$ (\acronymref{definition}{CBM}) is an upper triangular matrix.  By \acronymref{theorem}{SCB} we have
%
\begin{align*}
\matrixrep{S}{C}{C}&=\cbm{B}{C}\matrixrep{S}{B}{B}\inverse{\cbm{B}{C}}
\end{align*}
%
The inverse of an upper triangular matrix is upper triangular (\acronymref{theorem}{ITMT}), and the product of two upper triangular matrices is again upper triangular (\acronymref{theorem}{PTMT}).  So $\matrixrep{S}{C}{C}$ is an upper triangular matrix.\par
%
Now, multiply each vector of $C$ by a nonzero scalar, so that the result has norm 1.  In this way we create a new basis $D$ which is an orthonormal set (\acronymref{definition}{ONS}).  Note that the change-of-basis matrix $\cbm{C}{D}$ is a diagonal matrix with nonzero entries equal to the norms of the vectors in $C$.\par
%
Now we can convert our results into the language of matrices.  Let $E$ be the basis of $\complex{n}$ formed with the standard unit vectors (\acronymref{definition}{SUV}).  Then the matrix representation of $S$ relative to $E$ is simply $A$, $A=\matrixrep{S}{E}{E}$.  The change-of-basis matrix $\cbm{D}{E}$ has columns that are simply the vectors in $D$, the orthonormal basis.  As such, \acronymref{theorem}{CUMOS} tells us that $\cbm{D}{E}$ is a unitary matrix, and by \acronymref{definition}{UM} has an inverse equal to its adjoint.  Write $U=\cbm{D}{E}$.  We have
%
\begin{align*}
\adjoint{U}AU
...
\end{align*}
%
The inverse of a diagonal matrix is also a diagonal matrix, and so this final expression is the product of three upper triangular matrices, and so is again upper triangular (\acronymref{theorem}{PTMT}).  Thus the desired upper triangular matrix, $T$, is the matrix representation of $S$ relative to the orthonormal basis $D$, $\matrixrep{S}{D}{D}$.
%
\end{proof}
%
\subsect{NM}{Normal Matrices}
%
Normal matrices comprise a broad class of interesting matrices, many of which we have met already.  But they are most interesting since they define exactly which matrices we can diagonalize via a unitary matrix.  This is the upcoming \acronymref{theorem}{OD}.  Here's the definition.
%
\begin{definition}{NRML}{Normal Matrix}{normal matrix}
The square matrix $A$ is normal if $\adjoint{A}A=A\adjoint{A}$.
\end{definition}
%
So a normal matrix commutes with its adjoint.  Part of the beauty of this definition is that it includes many other types of matrices.  A diagonal matrix will commute with its adjoint, since the adjoint is again diagonal and the entries are just conjugates of the entries of the original diagonal matrix.  A Hermitian (self-adjoint) matrix (\acronymref{definition}{HM}) will trivially commute with its adjoint, since the two matrices are the same.  A real, symmetric matrix is Hermitian, so these matrices are also normal.  A unitary matrix (\acronymref{definition}{UM}) has its adjoint as its inverse, and inverses commute (\acronymref{theorem}{OSIS}), so unitary matrices are normal.  Another class of normal matrices is the skew-symmetric matrices.  However, these broad descriptions still do not capture all of the normal matrices, as the next example shows.
%
...
\end{align*}
%
so we see by \acronymref{definition}{NRML} that $A$ is normal.  However, $A$ is not symmetric (hence, as a real matrix, not Hermitian), not unitary, and not skew-symmetric.
\end{example}
%
\subsect{OD}{Orthonormal Diagonalization}
%
A diagonal matrix is very easy to work with in matrix multiplication (\acronymref{example}{HPDM}) and an orthonormal basis also has many advantages (\acronymref{theorem}{COB}).  How about converting a matrix to a diagonal matrix through a similarity transformation using a unitary matrix (i.e.\ build a diagonal matrix representation with an orthonormal matrix)?  That'd be fantastic!  When can we do this?  We can always accomplish this feat when the matrix is normal, and normal matrices are the only ones that behave this way.  Here's the theorem.
%
...
\end{align*}
%
So by \acronymref{definition}{NRML}, $A$ is a normal matrix.\par
%
($\Leftarrow$)\quad For the converse, suppose that $A$ is a normal matrix.   Whether or not $A$ is normal, \acronymref{theorem}{OBUTR} provides a unitary matrix $U$ and an upper triangular matrix $T$, whose diagonal entries are the eigenvalues of $A$, and such that $\adjoint{U}AU=T$.  With the added condition that $A$ is normal, we will determine that the entries of $T$ above the diagonal must be all zero.  Here we go.  First we show that $T$ is normal.
%
\begin{align*}
\adjoint{T}T
...
\end{align*}
%
So by \acronymref{definition}{NRML}, $T$ is a normal matrix.\par
%
...
%
The orthonormal basis formed from the columns of $U$ can be viewed as a system of mutually perpendicular axes.  The rotation by $\adjoint{U}$ allows the transformation by $A$ to be replaced by the simple transformation $D$ along these axes, and then $D$ brings the result back to the original coordinate system.  For this reason \acronymref{theorem}{OD} is known as the Principal Axis Theorem.  \index{principal axis theorem}\par
%
The columns of the unitary matrix in \acronymref{theorem}{OD} create an especially nice basis for use with the normal matrix.  We record this observation as a theorem.
%
...
%
\begin{proof}
Let $U$ be the unitary matrix promised by \acronymref{theorem}{OD} and let $D$ be the resulting diagonal matrix.  The desired set of vectors is formed by collecting the columns of $U$ into a set.  \acronymref{theorem}{CUMOS} says this set of columns is orthonormal.  Since $U$ is nonsingular (\acronymref{theorem}{UMI}), \acronymref{theorem}{CNMB} says the set is a basis.\par
%
Since $A$ is diagonalized by $U$, the diagonal entries of the matrix $D$ are the eigenvalues of $A$.  An argument exactly like the second half of the proof of \acronymref{theorem}{DC} shows that each vector of the basis is an eigenvector of $A$.
%
\end{proof}
%
In a vague way \acronymref{theorem}{OBNM} is an improvement on \acronymref{theorem}{HMOE} which said that eigenvectors of a Hermitian matrix for different eigenvalues are always orthogonal.  Hermitian matrices are normal and we see that we can find at least one basis where {\em every} pair of eigenvectors is orthogonal.  Notice that this is not a generalization, since \acronymref{theorem}{HMOE} states a weak result which applies to many (but not all) pairs of eigenvectors, while \acronymref{theorem}{OBNM} is a seemingly stronger result, but only asserts that there is one collection of eigenvectors with the stronger property.
%
%  End  OD.tex
